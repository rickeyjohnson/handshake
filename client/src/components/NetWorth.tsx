    ...
    <div className='rounded-xl border-1 border-stone-200 py-6 px-8 w-fit flex flex-col'>
      ...
      <div className='p-1 w-2xs'>
        <div className='flex'>
          ...
          <span className='font-medium'>{formatCurrency(userNetworth, true)}</span>
        </div>
        <div className='flex'>
          <p className='capitalize flex-1'>{user?.partner.name}'s net worth:</p>
          <span className='font-medium'>{formatCurrency(partnerNetworth, true)}</span>
        </div>
      </div>
    </div>
  )
